...
const SUPPORTED_OUTPUT_FORMATS = new Set(['png', 'jpeg', 'jpg', 'webp']);
...
Usage:
  ...
  node gpt_image_assets_cli.js generate --mode proxy --prompt "..." --image /path/to/reference.png --output out.png
...
Common generate options:
  ...
  --format png|jpeg|webp            alias for --output-format
...
function normalizeOutputFormat(args = {}) {
  ...
  if (!SUPPORTED_OUTPUT_FORMATS.has(normalized)) {
    throw new Error(`Unsupported output format: ${value}. Supported formats: png, jpeg, webp`);
  }
  return normalized;
}
...
async function commandGenerate(args) {
  ...
  validateOutputBuffer(result.imageBuffer, outputFormat);
