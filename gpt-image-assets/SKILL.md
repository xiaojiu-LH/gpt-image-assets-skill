...
description: "Generate PNG/JPEG/WebP images with GPT-Image-2 through official OpenAI or third-party Responses-compatible APIs. Use for text-to-image, image-to-image, ecommerce visuals, and output validation."
license: MIT
...
Use this Agent Skill when the user wants GPT-Image-2 image generation, image-to-image editing, ecommerce visuals, or PNG/JPEG/WebP files.
...
Default to PNG unless the user explicitly asks for JPEG or WebP.
...
PNG is the stable baseline. Official OpenAI image generation can produce PNG/JPEG/WebP. The CLI validates file signatures: PNG/JPEG/WebP must match their respective signatures.

If the user needs a layered PSD for Photoshop editing, use the companion `image-layer-psd` skill, which takes generated images and splits them into independent layers assembled into a multi-layer PSD.
...
- If the user needs exact product dimensions, logos, or text layout, recommend post-editing in Photoshop and treat generation as editable material, not final truth.
- If the user asks for layered PSD, recommend the `image-layer-psd` companion skill.
